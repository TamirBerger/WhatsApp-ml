...
def capture_and_save_pcap(output_path, interface, timeout, initial_time, BW):
    tshark_path = "C:\\Program Files\\Wireshark\\"
    duration = str(timeout)

    # Create a new pcap file
    tshark_cmd = f'"{tshark_path}tshark" -i {interface} -a duration:{duration} -w {output_path}\\pcap_{initial_time}_{BW}KBps.pcap'
...
def main():
    ...
    capture_and_save_pcap(dir_path, wifi_interface, timeout, initial_time, BW)
