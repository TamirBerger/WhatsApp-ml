...
class IP_UDP_ML:
    ...
    def __init__(self, feature_subset, estimator, config, metric, dataset, my_ip_l):
        ...
        self.pca = None
        self.variance_threshold = 0.9999
    ...
    def train(self, list_of_files):
        ...
        # Apply PCA and determine the number of components to retain the desired variance
        pca = PCA().fit(X)
        cumulative_explained_variance = np.cumsum(pca.explained_variance_ratio_)
        n_components = np.argmax(cumulative_explained_variance >= self.variance_threshold) + 1
        self.pca = PCA(n_components=12)
        X_transformed = self.pca.fit_transform(X)
        print(f'Number of components selected to retain {0.999 * 100}% variance: {n_components}')

        # Convert the transformed numpy array back to pandas DataFrame
        component_names = [f'PC{i + 1}' for i in range(self.pca.n_components_)]
        X = pd.DataFrame(X_transformed, columns=component_names)
        X = X.dropna()
