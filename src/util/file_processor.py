...
from util.validator import FileValidator
...
class FileProcessor:
    ...
    def get_in_lab_files(self):
        ...
        for experiment_dir in os.listdir(self.data_directory):
            ...
            try:
                csv_list = glob(f'{experiment_dir}/*.csv')
                labels_csv_file = glob(f'{experiment_dir}/*.json')[0]
            except:
                print('Missing file. Discarding the experiment')

            file_tuple = (csv_file, labels_csv_file)
