...
class KfoldCVOverFiles:
    ...
    def __init__(self, k, config, main_folder, net_conditions, metric):
        self.k = k
        self.config = config
    ...
    def split(self):
        # return a list of dict1 elements {net_cond_str: dict2}, when dict2 is {"train"/"test": list of file tuples}
        ...
        for net_cond in self.net_conditions:
            cond_folder = os.path.join(main_folder, net_cond)
            file_tuples_list = create_file_tuples_list(cond_folder, self.metric)
            ...
            for train_index, test_index in kf.split(X):
                ...
                cross_validation_splits[idx-1][net_cond]['train'] = X_train
                cross_validation_splits[idx-1][net_cond]['test'] = X_test
                print(
                    f'\nSplit # {idx} | net_cond = {net_cond} | n_files_train = {len(X_train)} | n_files_test = {len(X_test)}\n')
                idx += 1
