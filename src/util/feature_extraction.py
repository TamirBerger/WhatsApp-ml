...
class FeatureExtractor:
    ...
    def __init__(self, feature_subset, config):
        self.feature_subset = feature_subset
        self.config = config
        print('prediction window =', self.config['prediction_window'])
        self.max_size = -1  # max packets per 1 second (window size interval)
        self.max_iat = -1
    ...
    def extract_features(self, df_net):
        ...
        # Remove duplicate 'et'
        if len(features) > 1:
            for j in range(len(features)):  # remove 'et' from all df in features list except from the first
                if j > 0:
                    df = features[j]
                    df.drop(columns=['et'], inplace=True)
                    #df = df[df.columns.difference(['et'])]  # remove 'et' column from df
                    features[j] = df
    ...
    def extract_size_features(self, df_net):
        ...
        prev = int(df_net['time'].iloc[0])
        current_interval = 1
        end_times = []
        pkt_sizes = {'interval': [], 'sizes': []}
        n_features = self.config['n_features_size']  # max packets per second
        window_size = self.config['prediction_window']
        for i in range(df_net.shape[0]):
            #if df_net['time_normed'].iloc[i] - prev > window_size:  # if the delta between current time to prev bigger than 1 (window size)
            if i != 0 and int(df_net['time'].iloc[i]) - prev >= window_size:  # if advanced to next time window
                current_interval += 1
                # prev = df_net['time_normed'].iloc[i]
                prev = int(df_net['time'].iloc[i])
        ...
        for idx, row in df_agg.iterrows():
            ...
            if len(size_list) < n_features:
                size_list += [0]*(n_features-len(size_list))  # Complete size_list with zeros to n_features_size
            df_agg.at[idx, 'sizes'] = size_list[:n_features]
        size_col_names = [f'size_{i}' for i in range(1, n_features+1)]  # creates a list size_col_names containing column names for packet sizes
        df_size = pd.DataFrame(
            df_agg['sizes'].to_list(), columns=size_col_names)
        df_size['et'] = 0.0  # adds a new column 'et' to df_size and initializes all its values to 0.0.
        df_size.loc[:, 'et'] = end_times  # fill 'et' column with the end time of the interval
        df_size['et'] = df_size['et'].apply(lambda x: int(x))  # converts the values in 'et' column to integers
        return df_size
    ...
    def extract_iat_features(self, df_net):
        ...
        n_features = self.config['n_features_iat']
        ...
        for idx, row in df_agg.iterrows():
            iat_list = row['iats']
            self.max_iat = max(self.max_size, len(iat_list))  # max packet in interval
    ...
    def extract_length_stat_features(self, df_net):
        ...
        for i in range(df_net.shape[0]):
            ...
            if i != 0 and int(df_net['time'].iloc[i]) - prev >= window_size:  # if advanced to next time window
                ...
                prev = int(df_net['time'].iloc[i])
                end_times.append(df_net['time'].iloc[i])
            if i == df_net.shape[0]-1:  # if this is the last interval
                end_times.append(df_net['time'].iloc[i])
            pkt_sizes['interval'].append(current_interval)
            pkt_sizes['sizes'].append(df_net['length'].iloc[i])
        df_agg = pd.DataFrame(pkt_sizes)
        ...
        df_agg = df_agg.assign(**{'mean': 0, 'std': 0, 'min': 0, 'max': 0,
                               'q1': 0, 'q2': 0, 'q3': 0, 'num_pkts': 0, 'num_bytes': 0, 'num_unique': 0})
        for idx, row in df_agg.iterrows():
            d = defaultdict(int)
            sl = row['size_list']
            for x in sl:
